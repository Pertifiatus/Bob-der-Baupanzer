...
void CRSF::update() {
    while (_serial.available()) {
        ...
        // Synchronisation: Ein Frame beginnt immer mit 0xC8 (Receiver Address)
        if (crsfIndex == 0 && b != 0xC8) {
            continue; // Überspringe Bytes bis zum Start-Byte
        }

        crsfBuffer[crsfIndex++] = b;

        if (crsfIndex == 2) {
            // crsfBuffer[1] ist die Länge des restlichen Pakets
            // Ein RC-Paket ist inkl. Header und CRC meist 26 Bytes lang
        }

        if (crsfIndex >= 2) {
            uint8_t expectedLen = crsfBuffer[1];
            // Frame ist komplett (Header 2 Bytes + Rest)
            if (crsfIndex == expectedLen + 2) {
                parseCRSFFrame(crsfBuffer, expectedLen + 2);
                crsfIndex = 0;
            }
        }

        // Sicherheits-Reset bei Pufferüberlauf
...
void CRSF::parseCRSFFrame(uint8_t* frame, uint8_t length) {
    if (length < 14) return;
    if (frame[2] != 0x16 && frame[2] != 0x17) return; // nur RC-Kanäle
