...
const long HomingSpeedX = 100;
const long HomingSpeedY = 100;
const long HomingSpeedZ = 25;
const long HomingSpeedA = 750;
...
// Faehrt alle vier Achsen einzeln (kein Differenzial) mit fester Homing-
// Geschwindigkeit Richtung 0 (Ursprungslage). Gibt true zurueck, waehrend
// Homing aktiv ist - Stepper() darf dann in diesem Zyklus nicht zusaetzlich
// auf die Motoren schreiben.
bool Homing() {
  if (!(channels[7] > 500 && channels[4] < 500)) return false;

  long posX = Drehachse1X.currentPosition();
  long homeX = 0;
  if (posX > HomingToleranz) {
    homeX = -HomingSpeedX;
    if (posX < BREMSZONE_Drehachse1X) homeX = homeX * posX / BREMSZONE_Drehachse1X;
  } else if (posX < -HomingToleranz) {
    homeX = HomingSpeedX;
    if (-posX < BREMSZONE_Drehachse1X) homeX = homeX * -posX / BREMSZONE_Drehachse1X;
  }

  long posY = Gelenk2Y.currentPosition();
  long homeY = 0;
  if (posY > HomingToleranz) {
    homeY = -HomingSpeedY;
    if (posY < BREMSZONE_Gelenk2Y) homeY = homeY * posY / BREMSZONE_Gelenk2Y;
  } else if (posY < -HomingToleranz) {
    homeY = HomingSpeedY;
    if (-posY < BREMSZONE_Gelenk2Y) homeY = homeY * -posY / BREMSZONE_Gelenk2Y;
  }

  long posZ = Gelenk3Z.currentPosition();
  long homeZ = 0;
  if (posZ > HomingToleranz) {
    homeZ = -HomingSpeedZ;
    if (posZ < BREMSZONE_Gelenk3Z) homeZ = homeZ * posZ / BREMSZONE_Gelenk3Z;
  } else if (posZ < -HomingToleranz) {
    homeZ = HomingSpeedZ;
    if (-posZ < BREMSZONE_Gelenk3Z) homeZ = homeZ * -posZ / BREMSZONE_Gelenk3Z;
  }

  long posA = Drehachse4A.currentPosition();
  long homeA = 0;
  if (posA > HomingToleranz) {
    homeA = -HomingSpeedA;
    if (posA < BREMSZONE_Drehachse4A) homeA = homeA * posA / BREMSZONE_Drehachse4A;
  } else if (posA < -HomingToleranz) {
    homeA = HomingSpeedA;
    if (-posA < BREMSZONE_Drehachse4A) homeA = homeA * -posA / BREMSZONE_Drehachse4A;
  }

  digitalWrite(ENABLE_PIN, 0);  // Motoren fuers Homing aktivieren, unabhaengig von EN
  Drehachse1X.setSpeed(homeX);
  Gelenk2Y.setSpeed(homeY);
  Gelenk3Z.setSpeed(homeZ);
  Drehachse4A.setSpeed(homeA);
  Drehachse1X.runSpeed();
  Gelenk2Y.runSpeed();
  Gelenk3Z.runSpeed();
  Drehachse4A.runSpeed();
