  ...
  while (mySerial.available()) {
    ...
    if (incoming == '\n') {
      ...
      if (parsed == 3) {
        ...
        // Deadzone + Mapping
        c[0] = applyDeadzone(c[0], 150);
        c[0] = map(c[0], -1000, 1000, -SPEED, SPEED);

        c[1] = applyDeadzone(c[1], 100);
        c[1] = map(c[1], -1000, 1000, -SPEED, SPEED);

        c[2] = map(c[2], -1000, 1000, 0, 1);
...
void updateSteppers() {
  ...
  stepperX.setSpeed(c[0]);
  stepperX.runSpeed();

  stepperY.setSpeed(c[1]);
  stepperY.runSpeed();
