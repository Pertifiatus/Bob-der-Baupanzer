...
HardwareSerial CRSFSerial(1);  // UART1 für CRSF-Empfang
...
void setup() {
  ...
  CRSFSerial.begin(115200, SERIAL_8N1, 16, 17); // RX=16, TX=17 CRSF
  ArduinoSerial.begin(9600, SERIAL_8N1, 9, 10); // TX=10, RX=9 Arduino
...
void loop() {
  ...
  // Channels 1-4 an Arduino senden alle 50ms
...
// Channels 1-4 senden mit Startbyte an Arduino
void sendChannelsToArduino() {
  ArduinoSerial.write(START_BYTE);
  for (int i = 0; i < 4; i++) {
    ArduinoSerial.write((channels[i] >> 8) & 0xFF); // High Byte
    ArduinoSerial.write(channels[i] & 0xFF);        // Low Byte
  }
}
